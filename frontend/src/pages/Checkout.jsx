...
const PROMO_RATES = { ELITE20: 0.2, WELCOME10: 0.1 };
...
export default function Checkout() {
  ...
  const handleApplyPromo = () => {
    const rate = PROMO_RATES[promo.trim().toUpperCase()];
    if (rate) { setPromoRate(rate); setPromoApplied(true); addToast(`Promo code applied! ${Math.round(rate * 100)}% off`, 'success'); }
  ...
  const handlePay = async () => {
    ...
    try {
      const payload = {
        ...
        returnTime: state.returnTime || '17:00',
        insurance: !!(state.extras && state.extras.insurance),
        driverRequired: !!(state.extras && state.extras.extraDriver),
        numberOfDrivers: (state.extras && state.extras.extraDriver) ? 2 : 1,
        couponCode: promoApplied ? promo.trim().toUpperCase() : undefined,
      };
      ...
      if (result.backend) {
        try {
          await api.payments.create({ bookingId: result.id, method: 'CREDIT_CARD' });
        } catch (e) {
          addToast('Booking saved, but payment could not be recorded. Our team will contact you.', 'info');
        }
      }
  ...
  if (done) return (
    <main>
      <div className="success-screen">
        ...
        {bookingRef && <div className="booking-ref">Booking Reference: <strong>{bookingRef}</strong></div>}
      ...
      <style>{`
        ...
        .booking-ref { background: var(--dark-2); border: 1px solid var(--border); padding: 10px 20px; border-radius: 100px; font-size: 14px; color: var(--gray-1); }
      `}</style>
    </main>
  );
        ...
        <div className="checkout-layout">
          <div className="checkout-main">
            ...
            <div className="checkout-card">
              ...
              <div style={{ fontSize: 12, color: 'var(--gray-1)', marginTop: 8 }}>Try <strong style={{ color: 'var(--primary)' }}>ELITE20</strong> for 20% off or <strong style={{ color: 'var(--primary)' }}>WELCOME10</strong> for 10% off.</div>
            </div>
          ...
          <div className="checkout-summary">
            <div className="checkout-card">
              ...
              {promoApplied && <div className="order-row" style={{ color: 'var(--success)' }}><span>Promo ({promo.trim().toUpperCase()})</span><span>-GHS {discount}</span></div>}
              <div className="divider" />
